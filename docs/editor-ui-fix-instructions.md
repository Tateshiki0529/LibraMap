...
3. `QFrame`、`QGroupBox`、`QMessageBox`、`QDialog`、`QTableWidget`、`QHeaderView`、`QSpinBox`、`QComboBox` のサブコントロールまで指定する。
4. 黒背景を使う場合は、文字色を白に明示する。ただしEditorでは基本的に黒背景を避ける。
5. UI文言や機能追加は最小限にし、視認性修正に集中する。
...
4. 機能追加やレイアウト大改造は行わない。
5. 必要であれば右パネルの最小幅、フォーム余白、入力欄高さを微調整する。
6. 修正後に `docs/editor-manual-test-plan.md` の該当確認項目へ、視認性確認を追記する。
7. 自動テストと構文チェックを実行し、結果を報告する。
